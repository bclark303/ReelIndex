...
## Version 1.4.1 unified native container analysis

Version 1.4.1 combines the planned 1.4.1, 1.4.1, and 1.4.2 parser work into one release. Deep scans now use bounded native readers for Matroska/WebM, ISO Base Media and QuickTime (MP4/M4V/MOV), AVI/RIFF, ASF/WMV, MPEG transport streams (TS/M2TS/MTS), and MPEG program streams (MPG/MPEG). MP4-family files are sampled at both the beginning and end so a trailing `moov` atom can be read without traversing the full file. TS and PS readers sample bounded head/tail packet windows for program, stream, timing, and common codec metadata. Embedded iTunes-style MP4 cover artwork is cached as the movie poster when no sidecar is authoritative. ffprobe remains a bounded compatibility fallback for malformed, uncommon, or incomplete files.
